...
const StudentList = ({getRoleData, dataProcessing=null, role, columns, postObj, editObj}) => {
    ...
    const getData = ({params = {}}) => {
      setLoading(true);
      get(
        `${role}?page=${getParams(params).current}&limit=${
    ...
    const handleDelete = (id) => {
      apiDelete(`${role}/${id}`).then((response) => {
        message.success(response.msg);
        getData({ pagination });
      });
    };
